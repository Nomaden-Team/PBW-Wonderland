...
class UserController
{
    ...
    public function uploadFoto(): void
    {
        ...
        if ($error !== UPLOAD_ERR_OK) {
            header('Location: ' . $redirectBase . '&status=error&msg=' . urlencode('Upload gagal (kode error: ' . $error . ').'));
            exit;
        }
        ...
        $namaFile = 'foto_' . $userId . '_' . time() . '.' . strtolower($ext);
        ...
        if ($this->userModel->addUserPhoto($userId, $namaFile, $caption)) {
            header('Location: ' . $redirectBase . '&status=success&msg=' . urlencode('Foto berhasil diupload! Menunggu persetujuan admin.'));
            exit;
        }
    ...
    public function submitReservasi(): void
    {
        ...
        $kode = $this->reservasiModel->createByUser($userId, $namaKegiatan, $tanggal, $jumlah, $keterangan);

    ...
    <main class="adm-main">
        ...
        <div class="adm-page-header">
            <div>
                ...
                <p class="adm-page-sub">Manage semua reservasi pengunjung</p>
            </div>
        ...
        <div style="background:#D1FAE5;color:#065F46;padding:12px 18px;border-radius:10px;margin-bottom:18px;font-size:.85rem;font-weight:600;">
            ...
            <?= $_GET['ok'] === 'tambah' ? 'Reservasi berhasil ditambahkan!' : ($_GET['ok'] === 'edit' ? 'Reservasi berhasil diupdate!' : 'Reservasi berhasil dihapus!') ?>
        </div>
        <?php endif; ?>
        ...
        <div class="adm-card" style="margin-bottom:16px;padding:16px 20px;">
            <form method="GET" action="index.php" class="adm-filter-bar" style="margin:0">
                ...
                <div class="adm-search-wrap">
                    ...
                    <input type="text" name="search" class="adm-search-input" placeholder="Cari berdasarkan nama, email, atau ID..." value="<?= htmlspecialchars($search) ?>">
                </div>
                <select name="status" class="adm-select-filter" onchange="this.form.submit()">
                    <option value="">Semua Status</option>
                    <option value="terjadwal" <?= $filter_status==='confirmed'?'selected':'' ?>>Terjadwal</option>
                    <option value="selesai"   <?= $filter_status==='pending'?'selected':'' ?>>Selesai</option>
                    <option value="dibatalkan" <?= $filter_status==='cancelled'?'selected':'' ?>>Dibatalkan</option>
                </select>
            </form>
        </div>
...
<?php if (empty($reservasis)): ?>
    <tr><td colspan="7" class="adm-empty">Tidak ada data reservasi</td></tr>
<?php else: ?>
        ...
        if (!empty($r['user_id'])) {
            foreach ($user_list as $u) {
                if ((int)$u['id'] === (int)$r['user_id']) {
                    $namaUser = htmlspecialchars($u['nama']) . '<br><small style="color:#6b7280">' . htmlspecialchars($u['email']) . '</small>';
                    break;
                }
            }
        }
    ?>
    <tr>
        <td><strong>RES-<?= str_pad($r['id'], 3, '0', STR_PAD_LEFT) ?></strong></td>
        <td><?= $namaUser ?></td>
        <td>
        ...
        <td><span class="adm-badge adm-badge-<?= strtolower($r['status']) ?>"><?= ucfirst($r['status']) ?></span></td>
        <td>
            <div class="adm-td-actions">
                <button class="adm-btn-icon edit" onclick='openEditModal(<?= json_encode($r) ?>)' title="Edit">
                    <i class="far fa-edit"></i>
                </button>
                <form method="POST" action="index.php?page=admin_reservasi" onsubmit="return confirm('Hapus reservasi ini?')" style="display:inline">
                    <input type="hidden" name="action" value="hapus">
                    <input type="hidden" name="id" value="<?= $r['id'] ?>">
                    <button type="submit" class="adm-btn-icon del" title="Hapus"><i class="fas fa-trash"></i></button>
                </form>
            </div>
        </td>
    </tr>
        ...
        <form method="POST" action="index.php?page=admin_reservasi">
            ...
            <div class="adm-form-group">
                ...
                <select name="user_id" class="adm-search-input" style="padding-left:14px">
                    ...
                    <option value="<?= $u['id'] ?>"><?= htmlspecialchars($u['nama']) ?> (<?= htmlspecialchars($u['email']) ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="adm-form-group">

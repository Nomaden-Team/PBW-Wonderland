    ...
    <main class="adm-main">
        ...
        <div class="adm-page-header">
            <div>
                ...
                <p class="adm-page-sub">Manage ulasan & feedback pengunjung</p>
            </div>
        </div>
        ...
        <div class="adm-card" style="margin-bottom:20px;padding:16px 20px;">
            <form method="GET" action="index.php" class="adm-filter-bar" style="margin:0">
                ...
                <div class="adm-search-wrap">
                    ...
                    <input type="text" name="search" class="adm-search-input" placeholder="Cari ulasan..." value="<?= htmlspecialchars($search) ?>">
                </div>
            ...
            <div class="adm-ulasan-content">
                <div class="adm-ulasan-top">
                    <div>
                        ...
                        ?>
                        <p class="adm-ulasan-name"><?= htmlspecialchars($namaUser) ?>
                    ...
                    <span class="adm-badge adm-badge-<?= strtolower($statusUlasan) ?>"><?= ucfirst($statusUlasan) ?></span>
                </div>
                ...
                <div class="adm-ulasan-meta">
                    ...
                    <span><i class="far fa-thumbs-up"></i> <?= (int) ($ul['likes'] ?? 0) ?> likes</span>
                </div>
            </div>
            <div class="adm-ulasan-actions">
                <?php
                    $detailData = json_encode([
                        ...
                        'status' => $statusUlasan,
                        'date'   => $createdAt,
                        'likes'  => (int)($ul['likes'] ?? 0),
                    ], JSON_HEX_QUOT | JSON_HEX_APOS);
                ?>
                <button type="button" class="adm-btn-detail" onclick="showDetail(<?= htmlspecialchars($detailData, ENT_QUOTES) ?>)"><i class="far fa-eye"></i> Detail</button>

                <?php if ($statusUlasan === 'pending'): ?>
                <form method="POST" action="index.php?page=admin_ulasan">
                    <input type="hidden" name="action" value="approve">
                    <input type="hidden" name="id" value="<?= $ul['id'] ?>">
                    <button type="submit" class="adm-btn-approve" style="width:auto;padding:6px 14px;background:#16A34A">Approve</button>
                </form>
                <?php endif; ?>

                <?php if ($statusUlasan !== 'hidden'): ?>
                <form method="POST" action="index.php?page=admin_ulasan">
                    <input type="hidden" name="action" value="hide">
                    <input type="hidden" name="id" value="<?= $ul['id'] ?>">
                    <button type="submit" class="adm-btn-hide">Hide</button>
                </form>
                <?php else: ?>
                <form method="POST" action="index.php?page=admin_ulasan">
                    <input type="hidden" name="action" value="publish">
                    <input type="hidden" name="id" value="<?= $ul['id'] ?>">
                    <button type="submit" class="adm-btn-approve" style="width:auto;padding:6px 14px">Publish</button>
                </form>
                <?php endif; ?>
                <form method="POST" action="index.php?page=admin_ulasan" onsubmit="return confirm('Hapus ulasan ini?')">
                    <input type="hidden" name="action" value="hapus">
                    <input type="hidden" name="id" value="<?= $ul['id'] ?>">
                    <button type="submit" class="adm-btn-icon del"><i class="fas fa-trash"></i></button>
                </form>
            </div>
        </div>
        <?php endforeach; ?>
...
<div id="modalDetail" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;align-items:center;justify-content:center;">
  <div style="background:var(--adm-card,#1e293b);border-radius:12px;padding:28px 32px;max-width:520px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,.4);position:relative;color:var(--adm-text,#e2e8f0);">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:.93rem;">
      <tr><td style="padding:6px 0;color:#94a3b8;width:110px">Nama</td><td id="dNama" style="padding:6px 0;font-weight:600"></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8">Email</td><td id="dEmail" style="padding:6px 0"></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8">Wahana</td><td id="dWahana" style="padding:6px 0"></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8">Rating</td><td id="dRating" style="padding:6px 0"></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8">Status</td><td id="dStatus" style="padding:6px 0"></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8">Tanggal</td><td id="dDate" style="padding:6px 0"></td></tr>
      <tr><td style="padding:6px 0;color:#94a3b8">Likes</td><td id="dLikes" style="padding:6px 0"></td></tr>
    </table>
...
function showDetail(d) {
    ...
    document.getElementById('dLikes').textContent  = d.likes  + ' likes';
